                ...
                function activator(canvas, webGL, colorFormat, PROGRAMS, pointers) {
                    ...
                    function multipleSplats(amount) {
                        for (var i = 0; i < amount; i++) {
                            ...
                            color.g *= 10.0;
                            color.b *= 10.0;
                            var x = screen.width * (Math.random());
                            var y = screen.height * (Math.random());
                    ...
                    window.addEventListener('keydown', function (e) {
                        ...
                        for (const [key, value] of Object.entries(whiteKeys)) {
                            if (e.key === key) {
                                const currKey = document.querySelector(`.${key}`);
                                currKey.style.opacity = '80%';
                                splatStack.push(value);
                            }
                        }

                        for (const [key, value] of Object.entries(blackKeys)) {

                            if (e.key === key) {
                                let currKey;
                                if (blackKeyChr.includes(e.key)) {
                                    currKey = document.querySelector(`.${key}`);
                                } else {
                                    currKey = document.querySelector(`.b${key}`);
                                }
                                currKey.style.backgroundColor = 'gray';
                                splatStack.push(value);
                            }
                        }
                    });
                    ...
                    window.addEventListener('keyup', function (e) {
                        ...
                        for (const [key, value] of Object.entries(blackKeys)) {
                            if (e.key === key) {
                                ...
                                currKey.style.backgroundColor = 'black';
                            }
                        }
                    });

                    piano.addEventListener('click', function (e) {
                        splatStack.push(parseInt(Math.random() * 10));
                    })
                }
                /**
                 * Set Dither URL
